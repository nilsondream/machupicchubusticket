...
const paymentMethods = [
  { id: "paypal", name: "PayPal", taxes: "7%", icon: <Icons.PayPal /> },
  { id: "card", name: "Izipay", taxes: "5%", icon: <CreditCard /> },
  { id: "transfer", name: "Transfer", taxes: "No commission", icon: <Landmark /> },
];
        ...
        <div className="grid grid-cols-3 gap-3">
          {paymentMethods.map((method) => (
            <button
              key={method.id}
              type="button"
              onClick={() => setPaymentMethod(method.id as "paypal" | "card" | "transfer" | "none")}
              className={cn(
                ...
                method.id === "none" && "opacity-50 cursor-default bg-muted/50"
              )}
              disabled={method.id === "none"}
            >
